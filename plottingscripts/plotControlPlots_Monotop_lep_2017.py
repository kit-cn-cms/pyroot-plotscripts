...
def main(pyrootdir, opts):
    ...
    name = "Monotop_controlplots_lep_2017"
    ...
    # configs
    config = "Monotop/pltcfg_controlPlots_lep_2017"
    variable_cfg = "Monotop/additionalVariables"
    plot_cfg = "Monotop/controlPlots_lep_2017"
    ...
    with monitor.Timer("plotParallel"):
        ...
        pP.setCatNames([""])
        pP.setCatSelections(["(Evt_Pt_MET>100.)*(N_LoosePhotons==0)*((N_LooseElectrons+N_LooseMuons)==1)*((N_TightElectrons+N_TightMuons)==1)*(Jet_Pt[0]>80.)*(CaloMET_PFMET_ratio<1.0)*(DeltaPhi_AK4Jet_MET[0]>1.5)*(M_W_transverse[0]>=150.)*(N_Jets<=3)"])
