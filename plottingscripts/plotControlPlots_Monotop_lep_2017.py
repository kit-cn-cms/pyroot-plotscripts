...
def main(pyrootdir, opts):
    ...
    with monitor.Timer("plotParallel"):
        ...
        pP.setCatNames([""])
        pP.setCatSelections(["(Evt_Pt_MET>150.)*(N_LoosePhotons==0)*(M_W_transverse[0]>=50.)*((N_LooseElectrons+N_LooseMuons)==1)*((N_TightElectrons+N_TightMuons)==1)*(DeltaPhi_AK4Jet_MET[0]>2.0)"])
