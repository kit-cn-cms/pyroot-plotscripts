...
def main(argv):
    ...
    # definition of categories
    categoriesJP=[
              ("(N_Jets==4&&N_BTagsM>=3&&DNN_4j3t_pred_class==0)","j4_tge3_ttHnode",""),
              ("(N_Jets==4&&N_BTagsM>=3&&DNN_4j3t_pred_class==1)","j4_tge3_ttbbnode",""),
              ("(N_Jets==4&&N_BTagsM>=3&&DNN_4j3t_pred_class==2)","j4_tge3_tt2bnode",""),
              ("(N_Jets==4&&N_BTagsM>=3&&DNN_4j3t_pred_class==3)","j4_tge3_ttbnode",""),
              ("(N_Jets==4&&N_BTagsM>=3&&DNN_4j3t_pred_class==4)","j4_tge3_ttccnode",""),
              ("(N_Jets==4&&N_BTagsM>=3&&DNN_4j3t_pred_class==5)","j4_tge3_ttlfnode",""),
              
              ("(N_Jets==5&&N_BTagsM>=3&&DNN_5j3t_pred_class==0)","j5_tge3_ttHnode",""),             
              ("(N_Jets==5&&N_BTagsM>=3&&DNN_5j3t_pred_class==1)","j5_tge3_ttbbnode",""),             
              ("(N_Jets==5&&N_BTagsM>=3&&DNN_5j3t_pred_class==2)","j5_tge3_tt2bnode",""),             
              ("(N_Jets==5&&N_BTagsM>=3&&DNN_5j3t_pred_class==3)","j5_tge3_ttbnode",""),             
              ("(N_Jets==5&&N_BTagsM>=3&&DNN_5j3t_pred_class==4)","j5_tge3_ttccnode",""),             
              ("(N_Jets==5&&N_BTagsM>=3&&DNN_5j3t_pred_class==5)","j5_tge3_ttlfnode",""),             


              ("(N_Jets>=6&&N_BTagsM>=3&&DNN_6j3t_pred_class==0)","jge6_tge3_ttHnode",""),
              ("(N_Jets>=6&&N_BTagsM>=3&&DNN_6j3t_pred_class==1)","jge6_tge3_ttbbnode",""),
              ("(N_Jets>=6&&N_BTagsM>=3&&DNN_6j3t_pred_class==2)","jge6_tge3_tt2bnode",""),
              ("(N_Jets>=6&&N_BTagsM>=3&&DNN_6j3t_pred_class==3)","jge6_tge3_ttbnode",""),
              ("(N_Jets>=6&&N_BTagsM>=3&&DNN_6j3t_pred_class==4)","jge6_tge3_ttccnode",""),
              ("(N_Jets>=6&&N_BTagsM>=3&&DNN_6j3t_pred_class==5)","jge6_tge3_ttlfnode",""),

    ]
 
    # selections for categories
    categoriesJPsel="("+categoriesJP[0][0]
    for cat in categoriesJP[1:]:
      categoriesJPsel+="||"+cat[0]
    categoriesJPsel+=")"


    # category strings
    catstringJP="0"
    for i,cat in enumerate(categoriesJP):
        catstringJP+=("+"+str(i+1)+"*"+cat[0])
    ...
    plotsAdditional=[
        ...
        Plot(ROOT.TH1D("N_PrimaryVertices","Reconstructed primary vertices",51,-.5,50.5),"N_PrimaryVertices",plotselection,plotlabel),
        Plot(ROOT.TH1D("N_TruePV","True primary vertices",51,-.5,50.5),"N_GenPVs",plotselection,plotlabel),
    ...
    #discriminatorPlots+=[
                #Plot(ROOT.TH1D(discrname+"_ljets" ,"jet-process categories",len(categoriesJP),0.5,0.5+len(categoriesJP)),catstringJP,categoriesJPsel,"SL"),
#]
    ...
    #categorienames_MultiDNN=[
              ...
              #("(N_Jets>=4&&N_BTagsM>=2)","ljets_jge4_tge2",""),
...
#'Evt_HT_Jets',
    ...
    ## 3 and 4 tags
    #nhistobins_MultiDNN=[1]
    #minxvals_MultiDNN=[0   ]
    #maxxvals_MultiDNN=[3000   ]
    ...
    if (analysis.doDrawParallel==False or analysis.plotNumber != None) and analysis.makeSimplePlots==True :
        ...
        writeLOLAndOneOnTop(transposeLOL(lolT[1:]),samples[1:],lolT[0],samples[0],-1,name+'/'+name+'_controlplots')
        writeListOfHistoListsAN(transposeLOL([lolT[0]]+lolT[1:]),[samples[0]]+samples[1:],"",name+'/'+name+'_shapes',True,False,False,'histo',False,True,False)
    ...
    if (analysis.doDrawParallel==False or analysis.plotNumber != None) and analysis.makeMCControlPlots==True :
        ...
        plotDataMCanWsyst(listOfHistoListsData,transposeLOL(lolT[1:]),samples[1:],lolT[0],samples[0],-2,name,[[lll,3354,ROOT.kBlack,True]],False,labels,True,analysis.plotBlinded,verbosity=10)
        plotDataMCanWsyst(listOfHistoListsData,transposeLOL(lolT[1:]),samples[1:],lolT[0],samples[0],-2,name+'_LOG',[[lll,3354,ROOT.kBlack,True]],True,labels,True,analysis.plotBlinded,verbosity=10)
        #plotDataMCanWsyst(listOfHistoListsPseudoData,transposeLOL(lolT[9:]),samples[9:],lolT[0],samples[0],-2,name+'_PseudoData',[[lll,3354,ROOT.kBlack,True]],False,labels,True,analysis.plotBlinded,verbosity=10)
        #plotDataMCanWsyst(listOfHistoListsPseudoData,transposeLOL(lolT[9:]),samples[9:],lolT[0],samples[0],-2,name+'_PseudoData'+'_LOG',[[lll,3354,ROOT.kBlack,True]],True,labels,True,analysis.plotBlinded,verbosity=10)
