...
#memexp,
    ...
    for sample in samples:
        ...
        if sample.nick.startswith("ttbarPlus") or sample.nick == "ttbarOther":
                for ue_hdamp, ue_hdamp_file in zip(hdamp_ue_systnames_tt_all, hdamp_ue_filenames_tt_all):
                    if ue_hdamp.endswith("HDAMPUp"):
                        hotfixselection = thisnewsel + "*((N_GenTopHad==1 && N_GenTopLep==1)* %s + !(N_GenTopHad==1 && N_GenTopLep ==1)*1)" %str(round(1.0399, 2))
                    systsamples.append(Sample(sample.name+ue_hdamp,sample.color,ue_hdamp_file,hotfixselection,sample.nick+ue_hdamp,samDict=sampleDict))
    ...
    rename_systhisto_list = otherSystNames + hdamp_ue_systnames_tt_all + hdamp_ue_systnames_tt_lf+hdamp_ue_systnames_tt_cc+hdamp_ue_systnames_tt_b+hdamp_ue_systnames_tt_2b+hdamp_ue_systnames_tt_bb
