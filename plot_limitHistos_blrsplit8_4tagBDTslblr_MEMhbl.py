...
s4j4tlblr="(N_Jets==4&&N_BTagsM>=4 && TMath::Log(Evt_blr_ETH/(1-Evt_blr_ETH))<8.0)"
s4j4thblr="(N_Jets==4&&N_BTagsM>=4 && TMath::Log(Evt_blr_ETH/(1-Evt_blr_ETH))>=8.0)"
s5j3t="(N_Jets==5&&N_BTagsM==3)"
s5j4tlblr="(N_Jets==5&&N_BTagsM>=4 && TMath::Log(Evt_blr_ETH/(1-Evt_blr_ETH))<8.0)"
s5j4thblr="(N_Jets==5&&N_BTagsM>=4 && TMath::Log(Evt_blr_ETH/(1-Evt_blr_ETH))>=8.0)"
s6j2t="(N_Jets>=6&&N_BTagsM==2)"
s6j3t="(N_Jets>=6&&N_BTagsM==3)"
s6j4tlblr="(N_Jets>=6&&N_BTagsM>=4 && TMath::Log(Evt_blr_ETH/(1-Evt_blr_ETH))<8.0)"
s6j4thblr="(N_Jets>=6&&N_BTagsM>=4 && TMath::Log(Evt_blr_ETH/(1-Evt_blr_ETH))>=8.0)"
...
minxvals= [-0.89,-0.67,-0.56, -0.89,-0.67,-0.56]
maxxvals= [0.9,0.71,0.8, 0.9,0.71,0.8]
...
for b,bl,nb,minx,maxx in zip(bins,binlabels,nhistobins,minxvals,maxxvals):
  ...
  if bl=="jge6_tge4_hblr":
    bdts.append(Plot(ROOT.TH1F("BDT_ljets"+"_"+bl,"BDT_ljets"+" ("+bl+")",nb,0.0,1.0),"0.01*(MEM_p<0.0)+(MEM_p>=0.0)*MEM_p*MEM_p*MEM_p*MEM_p",b))
  if bl=="j5_tge4_hblr":
    bdts.append(Plot(ROOT.TH1F("BDT_ljets"+"_"+bl,"BDT_ljets"+" ("+bl+")",nb,0.0,1.0),"0.01*(MEM_p<0.0)+(MEM_p>=0.0)*MEM_p*MEM_p*MEM_p*MEM_p",b))
  if bl=="j4_t4_hblr":
    bdts.append(Plot(ROOT.TH1F("BDT_ljets"+"_"+bl,"BDT_ljets"+" ("+bl+")",nb,0.0,1.0),"0.01*(MEM_p<0.0)+(MEM_p>=0.0)*MEM_p*MEM_p*MEM_p*MEM_p",b))
