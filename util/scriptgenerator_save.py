...
void plot(){
  ...
  for transSample in sampleTranslationMapPython:
    retstr+="sampleTranslationMapCC[TString(\""+transSample+"\")]=TString(\""+sampleTranslationMapPython[transSample]+"\");\n"
...
def compileProgram(scriptname,usesDataBases,addCodeInterfaces):
  ...
  print cmdstring
  print ""
  print("writing compile command to "+scriptname+"_gccCommand.txt")
  compileOutFile=open(scriptname+'_gccCommand.txt',"w")
  compileOutFile.write(cmdstring+"\n")
  compileOutFile.close()
  print("trying to compile - output would be "+scriptname)
  try:
...
def createProgram(scriptname,plots,samples,catnames=[""],catselections=["1"],systnames=[""],allsystweights=["1"],additionalvariables=[],dataBases=[],addCodeInterfaces=[],MEPDFCSVFile=""):
  ...
  if MEPDFCSVFile!="":
    vetolist+=PDFutils.GetMEPDFVetoList(csv_file)
  ...
  if MEPDFCSVFile!="":
    script+=PDFutils.DeclareMEPDFNormFactors(csv_file)
    script+=PDFutils.AddMEandPDFNormalizationsMap(csv_file)
  ...
  if MEPDFCSVFile!="":
    script+=PDFutils.ResetMEPDFNormFactors(csv_file)
    script+=PDFutils.RelateMEPDFMapToNormFactor(csv_file)
    script+=PDFutils.PutPDFWeightsinVector(csv_file)
...
def get_scripts_outputs_and_nentries(samples,maxevents,scriptsfolder,plotspath,programpath,cmsswpath,treejsonfile="",cirun=False):
	...
	#print "will add it"
      ...
      if events_in_file > maxevents:
        for ijob in range(events_in_file/maxevents+1):
          ...
          samplewiseMaps[s.nick].append(outfilename)
  ...
  with open(scriptsfolder+'/'+"treejson.json","w") as jsonfile:
...
class plotParallel:
    def __init__(self, workdir, pltcfg, configData, samplesData):
        ...
        if pltcfg and configData and samplesData:
            self.__rootPath = self.__workdir + "/output.root"
            self.__analysisName = self.__workdir.split("/")[-1]

            self.__maxevents = 5000000
            self.__configData = configData
            self.__samplesData = samplesData
            self.__pltcfg = pltcfg

            self.__finished = False
            self.__haddFiles = None

            # defaults
            self.__catnames = [""]
            self.__catselections = ["1."]        
            
            self.__systnames = self.__pltcfg.weightSystNames
            self.__systweights = self.__pltcfg.systWeights

            self.__jsonFile = ""
            self.__dataBases = []
            self.__useDataBases = False
            self.__addInterfaces = []
            self.__MEPDFCSVFile = ""
            
            self.__options = {"cirun": False,
                            "stopAfterCompile": False,
                            "haddParallel": False,
                            "useOldRoot": False}
            
            ## check cmssw
            self.__cmsswpath = os.environ['CMSSW_BASE']
            if not "CMSSW" in self.__cmsswpath:
                print "you need CMSSW for this to work. Exiting!"
                exit(0)
            self.__cmsswversion = os.environ['CMSSW_VERSION']
    ...
    def setEmptyValues(self, path):
        ...
        if not os.path.exists(path):
            self.__limitPath = workdir + "/output.root"
        else:
            self.__limitPath = path
        self.__finished = True

        print("initialized empty plotParallel with limitPath set to: "+str(self.__limitPath))
    # ---------------------------------------------------------------------------------------------


    ## setters ####################################################################################
    def setCatnames(self, catnames):
        self.__catnames = catnames

    def setCatSelections(self, catselections):
        self.__catselections = catselections

    def setSystNames(self, systnames):
        self.__systnames = systnames
    
    def setSystWeights(self, systweights):
        self.__systweights = systweights
    ...
    def getHaddOutPath(self):
        return self.__workdir+"/HaddOutputs/*.root"
    
    def getHaddFiles(self):
        return self.__haddFiles

    def getLimitPath(self):
        return self.__limitPath
    ...
    def limitPath_Exists(self):
        self.__limitPath = self.__rootPath.replace(".root","_limitInput.root")
        return os.path.exists(self.__limitPath)
    
    ## end public functions #######################################################################
    ## private functions ##########################################################################

    def __globHaddFiles(self):
        allFiles = glob.glob(self.getHaddOutPath())
        allFiles = [f for f in allFiles if not "_renamed_" in f]
        return allFiles

    def __addHaddFiles(self):
        files = [self.__rootPath]
        if self.__haddFiles:
            files.extend(self.__haddFiles)
        return files

    def __writeCCInterface(self):
        print("#"*50)
        print("starting __writeCCInterface")
        print("c++ path is "+self.__ccPath)
        print("#"*50)
        ccExists = os.path.exists(self.__ccPath)
        if ccExists:
            print("c++ file already exists - check if this needs to be updated.")
            cmd = "cp -v "+self.__ccPath+" "+self.__ccPath+"Backup"
            print("cmd: "+cmd)
            subprocess.call(cmd, shell = True)

        print("creating c++ programm")
        # TODO update this function into this class
        createProgram(
            scriptname = self.__ccPath,
            plots = self.__configData.getDiscriminatorPlots(),
            samples = self.__samplesData.allSamples,
            catnames = self.__catnames,
            catselections = self.__catselections,
            systnames = self.__systnames,
            allsystweights = self.__systweights,
            additionalvariables = self.__configData.addVars,
            dataBases = self.__dataBases,
            addCodeInterfaces = self.__addInterfaces,
            MEPDFCSVFile = self.__MEPDFCSVFile)
        
        # check if createProgram was succesfull
        if not os.path.exists(self.__ccPath):
        ...
        if ccExists:
            ...
            codeDiffers = not filecmp.cmp(self.__ccPath+"Backup", self.__ccPath)
        if codeDiffers:
            print( "c++ codes differ" )
            print( "compiling c++ program" )
            # compiling programm
            # TODO move this to the class aswell?
            compileProgram(
                    scriptname = self.__ccPath[:-3],
                    usesDataBases = self.__useDataBases,
                    addCodeInterfaces = self.__addInterfaces)
        else:
            print( "c++ program already existing !! check if this is reasonable" )
            cmd = "cp -v "+self.__ccPath[:-3]+"Backup "+self.__ccPath[:-3]
            subprocess.call(cmd, shell = True)
        
        # check if compiling was successful
        if not os.path.exists(self.__ccPath[:-3]):
            print( "could not compile c++ program - exiting" )
            sys.exit(-1)
        print("#"*50)
        print(" done with __writeCCInterface ")
        print("#"*50)

    def __plotCheckJobs(self,scripts,outputs,nEntries):
        print("-"*50)
    ...
    def __haddCheckJobs(self, listOfOutScripts, listOfOutFiles):
        jobsToSubmit = []
        outfilesFromSubmit = []
        nJobsDone = 0
    ...
    def __haddSubmitInterface(self, listOfOutScripts, listOfOutFiles):
        firstTry = True
        nJobsDone = 0
        nTries = 0
        ...
        while len(jobsToSubmit) > 0 or len(outfilesFromSubmit) > 0 or nJobsDone != len(listOfOutFiles):
            ...
            if firstTry:
                ...
                jobids = submitArrayToNAF(jobsToSubmit, "haddPara")
                do_qstat(jobids)
                firstTry = False            
            else:
                print("hadding did not work the last time - resubmitting as single scripts")
                jobids = submitToNAF(jobsToSubmit)
                do_qstat(jobids)
            
            # check if each hadd worked
            jobsToSubmit, outfilesFromSubmit, nJobsDone = self.__haddCheckJobs(listOfOutScripts, listOfOutFiles)

    def __plotSubmitInterface(self, scripts, outputs, nEntries):
        # submit run scripts
        print 'submitting scripts'
        submitOptions = {"PeriodicHold": 1500}
        jobIDs = submitArrayToNAF(scripts, "PlotPara", submitOptions = submitOptions)
        # monitoring running of jobs
        do_qstat(jobIDs)

        # checking outputs
        print 'checking outputs'
        failedJobs = self.__plotCheckJobs(scripts,outputs,nEntries)
        retries=0
        maxRetries = 10
        while retries <= maxRetries and len(failedJobs)>0:
            ...
            jobIDs = submitToNAF(failedJobs, submitOptions = submitOptions)
            do_qstat(jobIDs)
            failedJobs = self.__plotCheckJobs(scripts,outputs,nEntries)
    ...
    def run(self):
        ...
        if os.path.exists(self.__rootPath):
            if self.__options["useOldRoot"]:
                if not self.__options["haddParallel"]:
                    ...
                    self.__haddFiles = self.__globHaddFiles()
                    self.__finished = True
                    return
                    
        # moving the old instance of workdir to a backup and copying C-file
        if os.path.exists(self.__rootPath):
            oldWorkdir = self.__workdir+datetime.datetime.now().strftime("%Y%m%d%H%M%S")
            os.rename(self.__workdir, oldWorkdir)
            os.makedirs(self.__workdir)

            cmd = "cp -v "+oldWorkdir+"/"+self.__analysisName+".cc "+self.__workdir+"/"+self.__analysisName+".cc"
            subprocess.call(cmd, shell = True)
            cmd = "cp -v "+oldWorkdir+"/"+self.__analysisName+" "+self.__workdir+"/"+self.__analysisName+"Backup"
            subprocess.call(cmd, shell = True)
        elif not os.path.exists(self.__workdir):
            os.makedirs(self.__workdir)

        # creating c++ programm
        self.__ccPath = self.__workdir + "/" + self.__analysisName + ".cc"
        self.__writeCCInterface()

        # create rename script
        # TODO look at rename script code and maybe also move to class
        createRenameScript(
                    scriptname = self.__ccPath[:-3],
                    systematics = self.__samplesData.allNames)
        
        # creating output folders
        print( "creating output folders" )
        self.__scriptsPath = self.__workdir + "/scripts/"
        self.__plotPath = self.__workdir + "/plots/"
        if not os.path.exists(self.__scriptsPath):
            os.makedirs(self.__scriptsPath)
        if not os.path.exists(self.__plotPath):
            os.makedirs(self.__plotPath)

        # creating run script
        # TODO move this to this class aswell and rename that shit
        print( "creating run scripts" )
        scripts, outputs, nentries, samplewiseMaps = get_scripts_outputs_and_nentries(
            samples = self.__samplesData.allSamples,
            maxevents = self.__maxevents,
            scriptsfolder = self.__scriptsPath,
            plotspath = self.__plotPath,
            programpath = self.__ccPath[:-3],
            cmsswpath = self.__cmsswpath,
            treejsonfile = self.__jsonFile,
            cirun = self.__options["cirun"])

        # check if we should stop
        if self.__options["stopAfterCompile"]:
            print( "compiling is done and stopAfterCompile option is activated - exiting" )
            sys.exit(0)

        # job submission
        self.__plotSubmitInterface(scripts, outputs, nentries)
        ...
        with runTimer.Timer("haddoutput"):
            if self.__options["haddParallel"]:
                # doing parallel hadding
                self.__doParaHadding(samplewiseMaps) 
                self.__finished = True
                return
            else:
                # not doing parallel hadding
                try:
                    rootFiles = self.__addHaddFiles()
                    subprocess.check_output(["hadd", rootFiles] + outputs, 
    ...
    ## run para hadding ###########################################################################
    def __doParaHadding(self, inmap = {}):
        # creating haddScript
        script = "import ROOT\n"
        script+= "import sys\n"
        script+= "import os\n"
        ...
        # saving hadd script
        self.__haddScript = self.__workdir+'/haddScript.py'
        with open(self.__haddScript, "w") as sf:
            sf.write(script)
        print("haddScript written to "+self.__haddScript)
        ...
        # initializing folder        
        self.__haddScriptFolder = self.__workdir+'/HaddScripts/'
        self.__haddOutputFolder = self.__workdir+'/HaddOutputs/'
        if not os.path.exists(self.__haddScriptFolder):
            os.makedirs(self.__haddScriptFolder)
        if not os.path.exists(self.__haddOutputFolder):
            os.makedirs(self.__haddOutputFolder)
        ...
        for sample in inmap:
            ...
            scriptname = self.__haddScriptFolder+'haddscript_'+sample+'.sh'
            haddedRootName = self.__haddOutputFolder+'/'+sample+'_hadded.root'
            haddedLogName = self.__haddOutputFolder+'/'+sample+'_hadded.log'
            script = "#!/bin/bash \n"
            if self.__cmsswpath!='':
                script += "export VO_CMS_SW_DIR=/cvmfs/cms.cern.ch \n"
                script += "source $VO_CMS_SW_DIR/cmsset_default.sh \n"
                script += "export SCRAM_ARCH="+os.environ['SCRAM_ARCH']+"\n"
                script += 'cd '+self.__cmsswpath+'/src\neval `scram runtime -sh`\n'
                script += 'cd - \n'
            script += 'python '+self.__haddScript+' '+haddedRootName+' '
            script += haddedLogName+' '+' '.join(inmap[sample])+'\n'
        ...
        # job submission
        self.__haddSubmitInterface(listOfOutScripts, listOfOutFiles)
        print("all jobs have terminated successfully")
        print("="*40)

        self.__haddFiles = listOfOutFiles
        return
