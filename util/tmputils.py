...
class configData:
        ...
        for index, row in self.Data.iterrows():
            discriminatorPlots.append(
                plotutils.Plot(
                    ...
                    label = row["binlabels"]))
        self.Data["discriminatorPlots"] = pandas.Series(
            discriminatorPlots, index = self.Data.index)

    def adjustDiscriminatorPlots(self, plotNumber):
        self.discriminatorPlot = [self.Data.discriminatorPlots[int(plotNumber)]]
        print("this is the new discriminatorPlot list:")
        print(self.discriminatorPlot)



    def getDiscriminatorPlots(self):
        try:
            return self.discriminatorPlot
        except:
            return self.Data.discriminatorPlots.values
    ...
    def getAdditionalDiscriminatorPlots(self, analysisClass, alwaysExecute=False):
        ...
        print self.Data.discriminatorPlots
        # adding plots to discriminator plots 
        evaluatedPlotList = [eval(plt) for plt in plotList]
        additionalPlotDict = {"discriminatorPlots": evaluatedPlotList}
        additionalData = pandas.DataFrame(additionalPlotDict)
        self.Data = self.Data.append(additionalData)
        print self.Data
        print self.Data.discriminatorPlots
