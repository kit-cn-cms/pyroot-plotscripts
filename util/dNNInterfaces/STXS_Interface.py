...
class DNN:
    def __init__(self, cpPath, opts, suffix = None):
        ...
        if type(cpPath) == dict:
            self.DNNs       = [DNN(
                    ...
                    for key in cpPath]
            self.multiDNN   = True
            self.xEval      = opts.crossEvaluation
            return
        ...
        if opts.dim:
            targetNodes = opts.targetNodes.split(",")
            #print("DNN suffix: {}, DNN cpPath: {}".format(self.suffix,self.cpPath))
        ...
        if not "binaryConfig" in self.config:
            ...
            for cls in classes:
                ...
                print("out_nodes:  {}".format(self.out_nodes))
            ...
            #print("DNN array: ".format(self.DNNs))
        else:
    ...
    def DoBinning(self,opts,i,label):
        string = ""
    ...
    def generateOutputPlots(self, opts):
        #print("cpPath: {}".format(self.cpPath))
        if self.multiDNN:
            string = ""
            secondClassifier_idx = 0
            for dnn in self.DNNs:
                print("DNN suffix: ".format(dnn.suffix))
                if opts.dim and dnn.suffix == opts.secondClassifier:
                    secondClassifier_idx = index(dnn)
                    print("Index of 2nd classifier: ".format(secondClassifier_idx))
            for dnn in self.DNNs:
    ...
    category_dict["nhistobins"] = ndefaultbins\n"""
                ...
                for i, node in enumerate(dnn.out_nodes):
                    label = "ljets_{cat}_{node}_node".format(cat = dnn.category, node = node)

                    # skip 2nd classifiers
                    if opts.dim and dnn.suffix == opts.secondClassifier:
                        continue

                    if dnn.xEval:
                        preselection = "({sel}&&({pred_var}=={i}))".format(
                            sel         = dnn.selection.replace(" ","").replace("and","&&"), 
                            pred_var    = dnn.predictionVariable, 
                            i           = i)
                    if opts.dim and node in opts.targetNodes:
                        for j, target in enumerate(self.DNNs[secondClassifier_idx].out_nodes):
                            print(dnn.suffix)
                            preselection = "({sel}&&(({pred_var_1}=={i})&&({pred_var_2}=={j}))".format(
                                sel         = dnn.selection.replace(" ","").replace("and","&&")+dnn.evalSelection,
                                pred_var_1  = dnn.predictionVariable,
                                i           = i,
                                pred_var_2  = self.DNNs[secondClassifier_idx].predictionVariable,
                                j           = j)
                            label = "ljets_{cat}_{target}_node".format(cat = dnn.category, target = target)
                            print(preselection)
                            string += template.format(  
                                PRESELECTION    = preselection,
                                LABEL           = label,
                                CATLABEL        = dnn.label,
                                DISCR_NAME      = dnn.discrNames[i])
                            string+=dnn.DoBinning(opts,i,label)
...
class theInterface:
    def __init__(self, workdir = None, optParser = None):
        # compile stuff
        self.includeString = includeString
        self.libraryString = libraryString
        self.usesPythonLibraries = True
        self.xEval = optParser.crossEvaluation

        if workdir:
            copy_tree(optParser.checkpoints, workdir+"/DNNCheckpoints/")
            optParser.checkpoints = workdir+"/DNNCheckpoints/"

        self.dnnSet = optParser.checkpoints
        ...
        for dnnDir in dnnDirs:
            ...
            if not errors:
                if len(dnnDict) == 1:
                    self.DNNs.append(DNN(dnnDict[dnnDict.keys()[0]], opts = optParser))
                else:
                    self.DNNs.append(DNN(dnnDict, opts = optParser))
                    print(dnnDict)
                    print(type(dnnDict))
                    print(self.DNNs)
            if len(self.DNNs) == 0:
...
no separate plots and no separate discriminators are produced""")
    ...
    parser.add_option("-2", "--twoDim", dest = "dim", action = "store_true", default = False,
        help = """Sets one classifier on top of another""")
    parser.add_option("-T", "--targetNodes", dest = "targetNodes", default = "ttH",
        help = """List of classifier nodes for further splitting""")
    parser.add_option("-S", "--secondClassifier", dest = "secondClassifier", default = "STXS",
        help = """Name of folder containing the checkpoints of the 2nd classifier""")
    ...
    interface = theInterface(optParser = opts)
