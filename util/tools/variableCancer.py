...
class Variable:
    ...
    def setupVariable(self, tree, verbose, variableManager):
        ...
        if self.varName == "GenEvt_I_TTZ":
            ...
            alt_file.Close()
        if "GenHiggs" in self.varName:
            self.inTree = True
            alt_file = ROOT.TFile("/nfs/dust/cms/user/swieland/ttH_legacy/ntuple/2017/ttHTobb_M125_TuneCP5_13TeV-powheg-pythia8/ttHTobb_M125_TuneCP5_13TeV-powheg-pythia8_6_nominal_Tree.root")
            alt_tree = alt_file.Get("MVATree")
            branch = alt_tree.GetBranch(self.varName)
            branchTitle = branch.GetTitle()
            self.setupVariableType(branchTitle, verbose)
