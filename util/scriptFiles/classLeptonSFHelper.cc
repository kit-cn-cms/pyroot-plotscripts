...
float LeptonSFHelper::GetElectronSF(float electronPt, float electronEta, int syst, std::string type)
{
    ...
    if (type == "ID") {
        ...
        if (electronPt < electronLowPtRangeCut) { current_reco_histo = h_ele_GFS_abseta_pt_ratio_lowEt; }
            ...
            current_reco_histo = h_ele_GFS_abseta_pt_ratio;
        }
...
void LeptonSFHelper::ChangeMuIsoHistos(bool is_DL)
{
    ...
    //     std::string ISOinputFileGtoH =
    //     "/nfs/dust/cms/user/kelmorab/DataFilesForScriptGenerator/Summer18_2017data/oct202017/mu_ISO_EfficienciesAndSF_GH.root";
...
{
    std::string IDinputFileBtoF =
        "/nfs/dust/cms/user/pkeicher/DataFilesForScriptGenerator/Summer18_2017data/Fall17/new_JEC/egammaEffi.txt_EGM2D_runBCDEF_passingTight94X.root";
    ...
    // std::string ISOinputFile = "/nfs/dust/cms/user/kelmorab/DataFilesForScriptGenerator/Summer18_2017data/oct202017/ele_Reco_EGM2D.root"; // DANGERZONE: no
    // iso SF yet??
    std::string GFSinputFile =
        "/nfs/dust/cms/user/pkeicher/DataFilesForScriptGenerator/Summer18_2017data/Fall17/new_JEC/egammaEffi.txt_EGM2D_runBCDEF_passingRECO.root";  // reco SFs
                                                                                                                                                    // for pt >
                                                                                                                                                    // 20
    std::string GFSinputFile_lowEt = "/nfs/dust/cms/user/pkeicher/DataFilesForScriptGenerator/Summer18_2017data/Fall17/new_JEC/"
                                     "egammaEffi.txt_EGM2D_runBCDEF_passingRECO_lowEt.root";  // reco SFs for pt<20
...
void LeptonSFHelper::SetMuonHistos()
{
    ...
    std::string TRIGGERinputFileBtoF =
        "/nfs/dust/cms/user/pkeicher/DataFilesForScriptGenerator/Summer18_2017data/Fall17/new_JEC/Muon_Trigger_SF_EfficienciesAndSF_RunBtoF_Nov17Nov2017.root";
...
{
    std::string TRIGGERinputFile =
        "/nfs/dust/cms/user/kelmorab/DataFilesForScriptGenerator/Summer18_2017data/oct202017/triggerSummary_mumu_ReReco2016_ttH.root";
...
float LeptonSFHelper::getValue(TGraphAsymmErrors &graph, float &x_, int syst)
{
    ...
    if (syst == 1) { return y_ + graph.GetErrorYhigh(i); }
        ...
        return y_ - graph.GetErrorYlow(i);
    }
    else {
        return y_;
    }
