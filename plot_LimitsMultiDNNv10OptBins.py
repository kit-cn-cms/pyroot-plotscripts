...
from plotconfigSpring17v10OptBin import *
...
if doDrawParallel==False or len(sys.argv) == 1 :                      #if some option is given old systematic histo file will be used      
    optimizeBinning(outputpath,name+'/'+name+'_optBinning.root',name+'/'+name+'_BinningConfig.txt', signalsamples=[samples[0]], backgroundsamples=samples[9:],additionalSamples=samples[1:9], plots=plots, systnames=weightSystNames+otherSystNames+PSSystNames, minBkgPerBin=2.0, optMode="SoverB",considerStatUnc=False, maxBins=50, minBins=2,verbosity=2)
...
if doDrawParallel==False or len(sys.argv) > 1 :
    listOfHistoLists=createHistoLists_fromSuperHistoFile(name+'/'+name+'_optBinning.root',samples,plots)    
    lolT=transposeLOL(listOfHistoLists)
    writeLOLAndOneOnTop(transposeLOL(lolT[9:]),samples[9:],lolT[0],samples[0],-1,name+'/'+name+'_controlplots_log',logscale=True)
    writeLOLAndOneOnTop(transposeLOL(lolT[9:]),samples[9:],lolT[0],samples[0],-1,name+'/'+name+'_controlplots',logscale=False)
