...
from plotconfig_v45 import *


def main(argv):

    #Create analysis object with output name
    name='limits_v45_michael'
    analysis=Analysis(name,argv,'/nfs/dust/cms/user/mwassmer/ttH_2018/pyroot-plotscripts/workdir/'+name+'/output_limitInput.root', signalProcess='ttH')
    ...
    for cats2D in plot2D_cats:
        ...
        for plot1 in plots:
            for plot2 in plots:
                ...
                plots2D.append(TwoDimPlot(ROOT.TH2D(name1+"_vs_"+name2,title1+"_"+title2+";"+title1+";"+title2,newBinsX,min1,max1,newBinsY,min2,max2),var1,var2,selection,label))
...
#memexp,
    ...
    if analysis.doDrawParallel==False or analysis.plotNumber == None :
        ...
        if not os.path.exists(analysis.rootFilePath):
            ...
            THEoutputpath=plotParallel(name,150000,discriminatorPlots,samples+samples_data+systsamples,[''],['1.'],weightSystNames,systWeights,additionalvariables,[["memDB","/nfs/dust/cms/user/kelmorab/DataBases/MemDataBase_ttH_2018",True]],"",otherSystNames,addCodeInterfacePaths=[],cirun=False,StopAfterCompileStep=False,haddParallel=True,useGenWeightNormMap=True,useThisSampleForVariableSetup=samples[9])
