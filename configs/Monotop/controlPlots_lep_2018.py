...
generalselection = "(N_HEM_Jets==0)*(N_HEM_METS==0)*(Evt_Pt_MET>100.)*(N_LoosePhotons==0)*(Jet_Pt[0]>80.)*(CaloMET_PFMET_ratio<1.0)*(DeltaPhi_AK4Jet_MET[0]>1.5)"
generalselection += "*(M_W_transverse[0]>=150.)"
...
def control_plots_lep_CR_ttbarEl(data=None):
    ...
    plots = [
        ...
        plotClasses.Plot(
            ROOT.TH1D("M_W_transverse" + extension, "m_{W,transverse}", len(discr_binning)-1, array('d',discr_binning)),
            "M_W_transverse",
            selection,
            label,
        ),
    ...
    if fast:
        ...
        plotClasses.Plot(
            ROOT.TH1D(
                "CaloMET_PFMET_ratio" + extension, "CaloMET_PFMET_ratio", 25, 0.0, 5.0
            ),
            "CaloMET_PFMET_ratio",
            selection,
            label,
...
def control_plots_lep_CR_ttbardilep(data=None):
    ...
    selection = generalselection
    selection += "*((N_LooseMuons==2 && N_TightMuons>=1 && N_LooseElectrons==0 && Triggered_HLT_IsoMu24_vX==1 && (DiMuon_Mass<60 || DiMuon_Mass>120)) || (N_LooseElectrons==2 && N_TightElectrons>=1 && N_LooseMuons==0 && (Triggered_HLT_Ele32_WPTight_Gsf_vX==1 || Triggered_HLT_Photon200_vX==1) && (DiElectron_Mass<60 || DiElectron_Mass>120)) || (N_LooseMuons==1 && N_LooseElectrons==1 && ((N_TightMuons==1 && Triggered_HLT_IsoMu24_vX==1) || (N_TightElectrons==1 && (Triggered_HLT_Ele32_WPTight_Gsf_vX==1 || Triggered_HLT_Photon200_vX==1)))))"
