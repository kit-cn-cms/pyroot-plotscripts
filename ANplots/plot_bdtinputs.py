...
plots+=[Plot(ROOT.TH1F("s43_BDT_common5_input_avg_btag_disc_btags","avg CSV (tags)",30,0.9,1.05),"BDT_common5_input_avg_btag_disc_btags","(N_Jets==4&&N_BTagsM==3)",label),
...
plots+=[Plot(ROOT.TH1F("s44_MEM_transformed","MEM discriminator",6,0,1.2),"(MEM_p>=0.0)*(MEM_p_sig/(MEM_p_sig+0.15*MEM_p_bkg))+(MEM_p<0.0)*(0.01)","(N_Jets==4&&N_BTagsM>=4)",label),
        Plot(ROOT.TH1F("s44_BDT_common5_input_avg_btag_disc_btags","avg CSV tagged",6,0.9,1.05),"BDT_common5_input_avg_btag_disc_btags","(N_Jets==4&&N_BTagsM>=4)",label),
        Plot(ROOT.TH1F("s44_BDT_common5_input_fourth_jet_pt","jet 4 p_{T}",7,30,100),"BDT_common5_input_fourth_jet_pt","(N_Jets==4&&N_BTagsM>=4)",label),
        ...
        Plot(ROOT.TH1F("s44_blr","B-tagging likelihood ratio",10,2,14),"TMath::Log(Evt_blr_ETH/(1-Evt_blr_ETH))",'(N_Jets==4&&N_BTagsM>=4)',label),
...
plots+=[Plot(ROOT.TH1F("s53_MEM_transformed","MEM discriminator",20,0,1),"(MEM_p>=0.0)*(MEM_p_sig/(MEM_p_sig+0.15*MEM_p_bkg))+(MEM_p<0.0)*(0.01)","(N_Jets==5&&N_BTagsM==3)",label),
        Plot(ROOT.TH1F("s53_BDT_common5_input_pt_all_jets_over_E_all_jets","(#sum jet p_{T})/(#sum jet E))",20,0.2,1.2),"BDT_common5_input_pt_all_jets_over_E_all_jets","(N_Jets==5&&N_BTagsM==3)",label),
...
plots+=[Plot(ROOT.TH1F("s54_MEM_transformed","MEM discriminator",10,0,1),"(MEM_p>=0.0)*(MEM_p_sig/(MEM_p_sig+0.15*MEM_p_bkg))+(MEM_p<0.0)*(0.01)","(N_Jets==5&&N_BTagsM>=4)",label),
        Plot(ROOT.TH1F("s54_BDT_common5_input_avg_btag_disc_btags","avg CSV (tags)",7,.9,1.04),"BDT_common5_input_avg_btag_disc_btags","(N_Jets==5&&N_BTagsM>=4)",label),
        ...
        Plot(ROOT.TH1F("s54_BDT_common5_input_avg_dr_tagged_jets","avg #Delta R (tag,tag)",10,1,4),"BDT_common5_input_avg_dr_tagged_jets","(N_Jets==5&&N_BTagsM>=4)",label),
        Plot(ROOT.TH1F("s54_blr","B-tagging likelihood ratio",10,3,15),"TMath::Log(Evt_blr_ETH/(1-Evt_blr_ETH))",'(N_Jets==5&&N_BTagsM>=4)',label),

    ]
# /nfs/dust/cms/user/kelmorab/newTrain/3makeHistosAndCards/weights/CommonWeights/weights_Final_54_v5_OldVars.xml
plots+=[Plot(ROOT.TH1F("s54_BDT_common5_input_pt_all_jets_over_E_all_jets","(#sum jet p_{T})/(#sum jet E))",10,0.2,1.2),"BDT_common5_input_pt_all_jets_over_E_all_jets","(N_Jets==5&&N_BTagsM>=4)",label),
...
plots+=[Plot(ROOT.TH1F("s62_BDT_common5_input_h1","H_{1}",27,-0.2,.34),"BDT_common5_input_h1","(N_Jets>=6&&N_BTagsM==2)",label),
        ...
        Plot(ROOT.TH1F("s62_BDT_common5_input_third_highest_btag","third highest CSV",22,0,1.1),"BDT_common5_input_third_highest_btag","(N_Jets>=6&&N_BTagsM==2)",label),
...
plots+=[Plot(ROOT.TH1F("s63_MEM_transformed","MEM discriminator",20,0.,1),"(MEM_p>=0.0)*(MEM_p_sig/(MEM_p_sig+0.15*MEM_p_bkg))+(MEM_p<0.0)*(0.01)","(N_Jets>=6&&N_BTagsM==3)",label),
        ...
        Plot(ROOT.TH1F("s63_BDT_common5_input_avg_btag_disc_btags","avg CSV (tags)",30,.9,1.05),"BDT_common5_input_avg_btag_disc_btags","(N_Jets>=6&&N_BTagsM==3)",label),
        Plot(ROOT.TH1F("s63_BDT_common5_input_avg_dr_tagged_jets","avg #Delta R (tag,tag)",28,0.5,3.9),"BDT_common5_input_avg_dr_tagged_jets","(N_Jets>=6&&N_BTagsM==3)",label),
...
plots+=[Plot(ROOT.TH1F("s64_BDT_common5_input_third_highest_btag","third-highest CSV",16,.89,1.05),"BDT_common5_input_third_highest_btag","(N_Jets>=6&&N_BTagsM>=4)",label),
        Plot(ROOT.TH1F("s64_MEM_transformed","MEM discriminator",10,0,1),"(MEM_p>=0.0)*(MEM_p_sig/(MEM_p_sig+0.15*MEM_p_bkg))+(MEM_p<0.0)*(0.01)","(N_Jets>=6&&N_BTagsM>=4)",label),
        Plot(ROOT.TH1F("s64_Evt_Deta_JetsAverage","avg #Delta #eta (jet,jet)",14,0,2.8),"Evt_Deta_JetsAverage","(N_Jets>=6&&N_BTagsM>=4)",label),
        ...
        Plot(ROOT.TH1F("s64_blr","B-tagging likelihood ratio",14,2,16),"TMath::Log(Evt_blr_ETH/(1-Evt_blr_ETH))",'(N_Jets>=6&&N_BTagsM>=4)',label),
...
plots+=[
        ...
        Plot(ROOT.TH1F("s64_BDT_common5_input_tagged_dijet_mass_closest_to_125","tagged dijet mass closest to 125",17,40,210),"BDT_common5_input_tagged_dijet_mass_closest_to_125","(N_Jets>=6&&N_BTagsM>=4)",label),
