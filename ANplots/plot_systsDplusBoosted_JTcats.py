...
categories_=[ ("(N_Jets==4&&N_BTagsM==3)&&!"+boosted,"ljets_j4_t3","4 jets, 3 b-tags"),
              ...
              ("(N_Jets==5&&N_BTagsM>=4)&&!"+boosted,"ljets_j5_tge4","5 jets, #geq4 b-tags "),
              ("(N_Jets>=6&&N_BTagsM==2)&&!"+boosted,"ljets_jge6_t2","#geq6 jets, 2 b-tags "),
              ("(N_Jets>=6&&N_BTagsM==3)&&!"+boosted,"ljets_jge6_t3","#geq6 jets, 3 b-tags "),
              ("(N_Jets>=6&&N_BTagsM>=4)&&!"+boosted,"ljets_jge6_tge4","#geq6 jets, #geq4 b-tags ")]
...
for cat,bdt in zip(categories_,bdtcuts):
  ...
  if "4 b-tags" in cat[2]:
    categories.append(('('+cat[0]+')*(BDT_common5_output>'+str(bdt)+')', cat[1]+'_high','#splitline{'+cat[2]+'}{ BDT > '+str(bdt)+'}') )
    categories.append(('('+cat[0]+')*(BDT_common5_output<='+str(bdt)+')', cat[1]+'_low','#splitline{'+cat[2]+'}{ BDT <= '+str(bdt)+'}') )
  else:
...
samples_nom=[Sample('t#bar{t}H',ROOT.kBlue+1,path_excl4252+'/ttH*/*nominal*.root',mcweight,'ttH') ,     
