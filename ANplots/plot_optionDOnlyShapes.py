...
name='76xBDToptionDdiscriminantsMEM'
...
allsystnames=[weightsystnames[0]]
systweights=[systweights[0]]
...
for discr,b,bl,nb,minx,maxx in zip(discrs,bins,binlabels,nhistobins,minxvals,maxxvals):
  if bl == "ljets_jge6_tge4_high":
    print "tst"
    helperbdts.append(MVAPlot(ROOT.TH1F("splitdummybdt"+"ljets_jge6_tge4","BDT for splitting ("+bl+")",nb*5,-1.0,1.0),bdtweightpath+'/weights_Final_64_76blr.xml',b))
  if bl == "ljets_j5_tge4_high":
    helperbdts.append(MVAPlot(ROOT.TH1F("splitdummybdt"+"ljets_j5_tge4","BDT for splitting ("+bl+")",nb*5,-1.0,1.0),bdtweightpath+'/weights_Final_54_76blr.xml',b))
  if bl == "ljets_j4_t4_high":
    helperbdts.append(MVAPlot(ROOT.TH1F("splitdummybdt"+"ljets_j4_t4","BDT for splitting ("+bl+")",nb*5,-1.0,1.0),bdtweightpath+'/weights_Final_44_76blr.xml',b))
  if '.xml' in discr:
...
outputpath=plotParallel(name,500000,bdts+helperbdts,samples,[''],['1.'],weightsystnames, systweights)
