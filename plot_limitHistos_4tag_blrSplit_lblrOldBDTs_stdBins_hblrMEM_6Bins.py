...
nhistobins=    [10,10,10,6,6,6]
minxvals= [-0.8,-0.86,-0.71, -0.8,-0.86,-0.71]
maxxvals= [0.89,0.9,0.65,0.89,0.9,0.65]
...
for b,bl,nb,minx,maxx in zip(bins,binlabels,nhistobins,minxvals,maxxvals):
  ...
  if bl=="jge6_tge4_hblr":
    bdts.append(Plot(ROOT.TH1F("BDT_ljets"+"_"+bl,"BDT_ljets"+" ("+bl+")",nb,0.0,1.0),"0.0001*(MEM_p<0.0)+(MEM_p>=0.0)*(MEM_p_sig/(MEM_p_sig+0.15*MEM_p_bkg))",b))
  if bl=="j5_tge4_hblr":
    bdts.append(Plot(ROOT.TH1F("BDT_ljets"+"_"+bl,"BDT_ljets"+" ("+bl+")",nb,0.0,1.0),"0.0001*(MEM_p<0.0)+(MEM_p>=0.0)*(MEM_p_sig/(MEM_p_sig+0.15*MEM_p_bkg))",b))
  if bl=="j4_t4_hblr":
    bdts.append(Plot(ROOT.TH1F("BDT_ljets"+"_"+bl,"BDT_ljets"+" ("+bl+")",nb,0.0,1.0),"0.0001*(MEM_p<0.0)+(MEM_p>=0.0)*(MEM_p_sig/(MEM_p_sig+0.15*MEM_p_bkg))",b))
