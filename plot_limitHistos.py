...
samples=[Sample('ttH125',ROOT.kBlue+1,path+'/ttH*/*nominal*.root',mcweight,'ttH125') ,     
         ...
         Sample('singlet',ROOT.kMagenta,path+'/st*/*nominal*.root',mcweight,'singlet') ,
         Sample('wjets',ROOT.kGreen-3,path+'/WJets*/*nominal*.root',mcweight,'wjets') ,
         Sample('zjets',ROOT.kAzure-3,path+'/Zjets*/*nominal*.root',mcweight,'zjets') ,
         Sample('ttbarZ',ROOT.kGray,path+'/ttZ*/*nominal*.root',mcweight,'ttbarZ') ,
         Sample('ttbarW',ROOT.kGray+5,path+'/ttW*/*nominal*.root',mcweight,'ttbarW') ,
         Sample('diboson',ROOT.kBlue-2,pathDB+'/DiBoson*/*nominal*.root',mcweight,'diboson') ,
]
...
for b,bl,nb in zip(bins,binlabels,nhistobins):
    bdts.append(Plot(ROOT.TH1F("BDT_ljets"+"_"+bl,"BDT_ljets"+" ("+bl+")",nb,-1,1),"BDT_common5_output",b))
