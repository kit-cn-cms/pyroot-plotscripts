...
def main(argv):
    ...
    plotsAdditional=[
        ...
        #Plot(ROOT.TH1DD("pt3tagged","p_{T} of third tagged jet",40,0,400),"TaggedJet_Pt[2]",plotselection,plotlabel),
    ...
    #plotsToBe2Dalized=plots64+plots63+plots54+plots53+plots44+plots43
    plotsToBe2Dalized=plots64
    print "plotsToBe2Dalized"
    print len(plotsToBe2Dalized)
    seenCombinations=[]
    plots2D=[]
    for plot1 in plotsToBe2Dalized:
        for plot2 in plotsToBe2Dalized:
            # skip same plots
            if plot1==plot2:
            ...
            plots2D.append(TwoDimPlot(ROOT.TH2D(name1+"_vs_"+name2,title1+"_vs_"+title2+";"+title1+";"+title2,newBinsX,min1,max1,newBinsY,min2,max2),var1,var2,selection,label))
            
    print "number of 2D plots"
    print len(plots2D)
    #exit(0)
    plots=[]
    plots=plots2D
    ...
    if analysis.doDrawParallel==False or analysis.plotNumber == None :
        if not os.path.exists(analysis.rootFilePath):
            ...
            outputpath=plotParallel(name,500000,discriminatorPlots,samples+samples_data+systsamples,[''],['1.'],weightSystNames,systWeights,additionalvariables,[],"",otherSystNames+PSSystNames,addCodeInterfacePaths=[],cirun=False,StopAfterCompileStep=False)
