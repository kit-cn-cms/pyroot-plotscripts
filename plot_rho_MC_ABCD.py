...
name='rho_MC_'+ABCDeventhandling+'_'+ABCDversion+radi+'_'+WPs
...
if not os.path.isfile(rebinnedHistoPath):
    ...
    createBRLLL(transposeLOL(transposeLOL(lllSignal)[SignalSampleNames.index("SigRho1500700_"+BR_name+""):SignalSampleNames.index("SigRho25001500_"+BR_name+"")+1]),transposeLOL(transposeLOL(lllSignal)[SignalSampleNames.index("SigRho1500700_tWb"+""):SignalSampleNames.index("SigRho25001500_tWb"+"")+1]),transposeLOL(transposeLOL(lllSignal)[SignalSampleNames.index("SigRho1500700_ttZ"+""):SignalSampleNames.index("SigRho25001500_ttZ"+"")+1]),transposeLOL(transposeLOL(lllSignal)[SignalSampleNames.index("SigRho1500700_ttH"+""):SignalSampleNames.index("SigRho25001500_ttH"+"")+1]),BR[0],BR[1],BR[2])
    
    #createBRLLL(transposeLOL(transposeLOL(lllSignal)[SignalSampleNames.index("SigRho1500800_tWb"+BR_name+"WID"):SignalSampleNames.index("SigRho1500800_"+BR_name+"WID")+1]),transposeLOL(transposeLOL(lllSignal)[SignalSampleNames.index("SigRho1500800_tWb"+"WID"):SignalSampleNames.index("SigRho40003000_tWb"+"WID")+1]),transposeLOL(transposeLOL(lllSignal)[SignalSampleNames.index("SigRho1500800_ttZ"+"WID"):SignalSampleNames.index("SigRho40003000_ttZ"+"WID")+1]),transposeLOL(transposeLOL(lllSignal)[SignalSampleNames.index("SigRho1500800_ttH"+"WID"):SignalSampleNames.index("SigRho40003000_ttH"+"WID")+1]),BR[0],BR[1],BR[2])  ,
...
for l1,l2,l3,signame in zip(lllSignal[plotnames.index(ABCDversion + "_withtopbtag_CatA_Zprime_eff")],lllSignal[plotnames.index(ABCDversion + "_notopbtag_CatA_Zprime_eff")],lllSignal[plotnames.index("N_Gen_ZPrimes")], SignalSampleNames):
    ...
    if len(l1)>0 and len(l2)>0 and len(l3)>0:
        if l1[allweightsystnames.index('_'+ABCDversion+'_nominal')].Integral()>0 and l2[allweightsystnames.index('_'+ABCDversion+'_nominal')].Integral()>0 and l3[allweightsystnames.index('_'+ABCDversion+'_nominal')].Integral()>0:
            ...
            #ZprimeToTprimeT_TprimeToWB_MZp-1500Nar_MTp-700Nar_LH
                
            filename=glob.glob("/nfs/dust/cms/user/skudella/processed_MC/scripts/cutflow/*"+decay+"*MZp-"+mZ+width+"*"+mT+"*LH*.txt")
            #print filename
            f=open(filename[0],"r")
            for line in f:
                if ("0 : all : " in line):
...
f=open("signal efficiency_Rho.txt",'w')
f.write(text)
f.close()
